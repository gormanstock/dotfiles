...
const lens_collection_1 = __webpack_require__(/*! ../lens-collection */ "../vscode-js-profile-core/out/lens-collection.js");
...
class CpuProfileEditorProvider {
    ...
    createLensCollection(document) {
        var _a, _b;
        const lenses = new lens_collection_1.LensCollection(dto => {
            let title;
        ...
        const merge = (location) => (existing) => ({
            ticks: ((existing === null || existing === void 0 ? void 0 : existing.ticks) || 0) + location.ticks,
            self: ((existing === null || existing === void 0 ? void 0 : existing.self) || 0) + location.selfTime,
            agg: ((existing === null || existing === void 0 ? void 0 : existing.agg) || 0) + location.aggregateTime,
        });
        for (const location of ((_a = document.userData) === null || _a === void 0 ? void 0 : _a.locations) || []) {
            const mergeFn = merge(location);
            lenses.set(location.callFrame.url, new vscode.Position(Math.max(0, location.callFrame.lineNumber), Math.max(0, location.callFrame.columnNumber)), mergeFn);
            const src = location.src;
            if (!src || src.source.sourceReference !== 0 || !src.source.path) {
                continue;
            }
            for (const path of open_location_1.getCandidateDiskPaths((_b = document.userData) === null || _b === void 0 ? void 0 : _b.rootPath, src.source)) {
                lenses.set(path, new vscode.Position(Math.max(0, src.lineNumber - 1), Math.max(0, src.columnNumber - 1)), mergeFn);
            }
        }
        return lenses;
    }
...
exports.buildModel = (profile) => {
    ...
    for (let i = 0; i < profile.nodes.length; i++) {
        ...
        nodes[node.id - 1] = {
            ...
            children: ((_b = node.children) === null || _b === void 0 ? void 0 : _b.map(n => n - 1)) || [],
        };
...
/***/ "../vscode-js-profile-core/out/lens-collection.js":
/*!********************************************************!*\
  !*** ../vscode-js-profile-core/out/lens-collection.js ***!
  \********************************************************/
/*! no static exports found */
...
class LensCollection {
    ...
    set(file, position, factory) {
        ...
        if (((_a = list[index]) === null || _a === void 0 ? void 0 : _a.position.line) === position.line) {
            ...
            position = new vscode_1.Position(position.line, Math.min(list[index].position.character, position.character));
        }
    ...
    getDtoAt(file, line) {
        var _a, _b;
        return (_b = (_a = this.data.get(path_1.lowerCaseInsensitivePath(file))) === null || _a === void 0 ? void 0 : _a.find(p => p.position.line === line)) === null || _b === void 0 ? void 0 : _b.dto;
    }
    /**
    ...
    getLensesForFile(file) {
        ...
        return ((_b = (_a = this.data
            .get(path_1.lowerCaseInsensitivePath(file))) === null || _a === void 0 ? void 0 : _a.map(l => {
            const command = this.mapper(l.dto);
            return command && new vscode_1.CodeLens(new vscode_1.Range(l.position, l.position), command);
        }).filter((cmd) => !!cmd)) !== null && _b !== void 0 ? _b : []);
    }
    /**
...
exports.LensCollection = LensCollection;
//# sourceMappingURL=lens-collection.js.map
...
class ProfileCodeLensProvider {
    ...
    provideCodeLenses(document) {
        var _a, _b;
        const byPath = (_a = this.lenses) === null || _a === void 0 ? void 0 : _a[path_1.lowerCaseInsensitivePath(document.uri.fsPath)];
        if (byPath) {
            return byPath;
        }
        const byUrl = document.uri.scheme === download_file_provider_1.DownloadFileProvider.scheme
            ? (_b = this.lenses) === null || _b === void 0 ? void 0 : _b[document.uri.query] : undefined;
        if (byUrl) {
            return byUrl;
        }
        return [];
    ...
    setLenses(collection) {
        ...
        for (const file of collection.files()) {
            ...
            for (const lens of collection.getLensesForFile(file)) {
                lenses.push(lens, new vscode_1.CodeLens(lens.range, {
                    title: 'Clear',
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerCustomEditorProvider2('jsProfileVisualizer.cpuprofile.table', new editorProvider_1.CpuProfileEditorProvider(lenses, path_1.join(__dirname, 'client.bundle.js'))), vscode.workspace.registerTextDocumentContentProvider('js-viz-download', new download_file_provider_1.DownloadFileProvider()), vscode.languages.registerCodeLensProvider('*', lenses), vscode.commands.registerCommand('extension.jsProfileVisualizer.table.clearCodeLenses', () => lenses.clear()));
}
