...
function activate(context) {
    logging_1.setupLoggingOutputChannel(context);
...
function onTaskProcessEnd(event) {
    ...
    if (bazelTaskInfo) {
        ...
        const exitCode = bazellib_1.parseExitCode(rawExitCode, bazelTaskInfo.command);
        if (rawExitCode !== 0) {
            vscode.window.showErrorMessage(`Bazel ${bazelTaskInfo.command} failed: ${bazellib_1.exitCodeToUserString(exitCode)}`);
        }
        else {
