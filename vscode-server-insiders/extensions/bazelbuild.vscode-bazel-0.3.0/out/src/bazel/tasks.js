...
const bazel_task_info_1 = require("./bazel_task_info");
/**
...
function createBazelTask(command, options) {
    ...
    const task = new vscode.Task({ type: "bazel", command, targets: options.targets }, 
    // TODO(allevato): Change Workspace to Global once the fix for
    // Microsoft/vscode#63951 is in a stable release.
    options.workspaceInfo.workspaceFolder || vscode.TaskScope.Workspace, `${commandDescription} ${targetsDescription}`, "bazel", new vscode.ShellExecution(configuration_1.getDefaultBazelExecutablePath(), args, {
        cwd: options.workspaceInfo.bazelWorkspacePath,
    }));
    bazel_task_info_1.setBazelTaskInfo(task, new bazel_task_info_1.BazelTaskInfo(command, options));
    return task;
}
